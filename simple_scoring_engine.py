#!/usr/bin/env python3
"""
Simple Resume Relevance Scoring Engine (No Dependencies)
...
class SimpleRelevanceScorer:
    """Lightweight scoring engine for resume-JD matching."""
    ...
    def semantic_similarity_score(self, resume_text: str, jd_text: str) -> float:
        ...
        # Also consider word count ratio
        length_ratio = min(len(resume_words), len(jd_words)) / max(len(resume_words), len(jd_words)) if max(len(resume_words), len(jd_words)) > 0 else 0
        
        # Combine both metrics
        semantic_score = (jaccard * 0.7 + length_ratio * 0.3) * 100
...
def demo_scoring():
    ...
    jd = JobDescription(
        ...
        description="We are looking for an experienced Python developer to build scalable web applications using Django and PostgreSQL."
    )
        ...
        'experience': [
            {
                ...
                'company': 'Tech Corp',
                'bullets': ['Developed Python applications for 3 years', 'Built REST APIs using Django']
            }
        ],
        'education': [
            {
        ...
        'experience': [
            {
                ...
                'bullets': ['Did some web development for 6 months']
            }
        ],
        'education': [
            {
    ...
    weak_result = scorer.calculate_relevance_score(weak_resume, jd)
    print(f"\nLow Match Example:")
    print(f"Candidate: {weak_resume['name']}")
    print(f"Score: {weak_result.relevance_score}/100 ({weak_result.verdict})")
    
    # Test high-scoring profile
    strong_resume = {
        'name': 'Alex Johnson',
        'skills': ['Python', 'Django', 'PostgreSQL', 'REST API', 'Docker', 'AWS', 'React', 'Redis', 'Kubernetes'],
        'experience': [
            {
                'title': 'Senior Python Developer',
                'company': 'Big Tech Corp',
                'bullets': ['Led Python development for 5 years', 'Built scalable Django applications', 'Managed AWS infrastructure']
            }
        ],
        'education': [
            {
    ...
    strong_result = scorer.calculate_relevance_score(strong_resume, jd)
    print(f"\nHigh Match Example:")
    print(f"Candidate: {strong_resume['name']}")
    print(f"Score: {strong_result.relevance_score}/100 ({strong_result.verdict})")
